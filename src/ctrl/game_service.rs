...
impl GameService {
    ...
    async fn query_game_release_reports(&self, pool: &Pool<MySql>, params: &ReqQueryReports, conds: &Vec<String>) -> Option<Vec<ResAdsGameReports>>{
        ...
        sql += format!(") t 
        LEFT JOIN ( SELECT stat_datetime, SUM(earnings) AS earnings, app_id, CAST(SUM(reached_ad_requests) as SIGNED) as reached_ad_requests, CAST(SUM(click_count) as SIGNED) as click_count, CAST(SUM(matched_reached_ad_requests) as SIGNED) as matched_reached_ad_requests, CAST(SUM(show_count) as SIGNED) as show_count FROM ads_daily_earnings_reports WHERE {} GROUP BY app_id, stat_datetime ) t2
        ON t.app_id = t2.app_id and t.stat_datetime = t2.stat_datetime
    ...
    pub async fn get_game_ad_type_reports(&self, pool: &Pool<MySql>, params: &ReqQueryAdTypeReports) -> Option<Vec<ResAdsGameAdTypeReports>> {
        let rs = sqlx::query_as::<_, ResAdsGameAdTypeReports>("select * from ads_daily_earnings_reports where app_id = ? and stat_datetime = ? and ad_type != 'ALL'")
        .bind(&params.app_id)
    ...
    pub async fn get_roas_period_data(&self, pool: &Pool<MySql>, params: &ReqGetPeroidRoas) -> Option<Vec<ResPeriodRoas>> {
        let rs = sqlx::query_as::<_, ResPeriodRoas>("select * from ads_period_release_reports where package_name = ? and date between ? and ?")
        ...
        match rs {
            Ok(v) => Some(v),
            Err(e) => {
                println!("get_roas_period_data {}", e);
                None
            }
        }
    }
