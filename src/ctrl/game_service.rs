...
impl GameService {
    ...
    pub async fn get_reports(&self, pool: &Pool<MySql>, params: &ReqQueryReports) -> Option<ResGetReports> {
        ...
        let mut sql = "SELECT a.package_name, a.cost, a.active, a.iaa, DATE_FORMAT(a.stat_datetime , '%Y-%m-%d') as stat_datetime, b.app_name, c.earnings, d.iaa as first_day_iaa, f.duration, f.r1 
    ...
    async fn calc_release_daily_reports(&self, pool: &Pool<MySql>, advertiser_id: &String, today: &String, record_date: &String, app_package_names: &mut HashSet<String>) {
        ...
        if let Some(list) = list {
            println!("calc_release_daily_reports: {} {}", today, record_date);
            for vo in list {
